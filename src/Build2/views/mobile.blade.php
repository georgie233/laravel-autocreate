    ...
    <div class="card" id="app">
        ...
        <div class="tab-container">
            <ul role="tablist" class="nav nav-tabs">
                <li class="nav-item"><a href="#" class="nav-link active">{MODEL_TITLE}列表</a></li>
                <li class="nav-item"><a href="{{ route("{SMODULE}_{SMODEL}.create") }}" class="nav-link">添加{MODEL_TITLE}</a></li>
            </ul>
                            ...
                            <div class="card-body">
                                ...
                                <div id="coll{{$d['id']}}" class="collapse">
                                    ...
                                    <div class="card-footer m-0 p-1 border-0 text-right">
                                        <a href="{{ route("{SMODULE}_{SMODEL}.edit",$d['id']) }}" class="btn btn-secondary">编辑</a>
                                        <button type="button" class="btn btn-secondary btn-danger"
                                                onclick="del({{$d['id']}},this)">删除
                                        </button>
                                        <form action="{{ route("{SMODULE}_{SMODEL}.destroy",$d['id']) }}" hidden method="post">
                                            @csrf @method('DELETE')
                                        </form>
                                    </div>
                                </div>
